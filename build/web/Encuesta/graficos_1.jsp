    ...
    <head>
        ...
        <script type="text/javascript" src="https://www.gstatic.com/charts/loader.js"></script>
         <script type="text/javascript">
            ...
            //Dibujar las graficas cuando charts cargen
            <%for (int pregunta = 0; pregunta < preguntas.length; pregunta++) 
            {
                ...
                {%>
                    //Dibujar cada una de las graficas, el callback de la grafica se llamara igual que su ID
                    google.charts.setOnLoadCallback(draw<%=preguntas[pregunta].getId_preguntas()%>);
            <% }
            }
            %>
              
                
            //Crear los callbacks de cada una de las graficas 
            <%for (int pregunta = 0; pregunta < preguntas.length; pregunta++) 
            {
                if (preguntas[pregunta].getTipo().compareTo("Abierta") != 0)//Si la pregunta no es abierta...
                {%>
                    //La funcion se llamara como el ID de la pregunta
                    function draw<%=preguntas[pregunta].getId_preguntas()%>() 
                    {

                        // Crear la tabla para las respuestas de la pregunta
                        var data = google.visualization.arrayToDataTable([
                            ['Respuesta', 'Alumnos']
                        //preguntas[pregunta]
                        <%
                        for(int respuesta=0; respuesta<preguntas[pregunta].getNum_respuestas(); respuesta++)
                        { %>
                            ,['<%=preguntas[pregunta].respuestas[respuesta]%>', <%=respuestas[pregunta].respuestas[respuesta]%>]
                        <%
                        }
                        %>
                        ]);

                        // Cambiar opciones para el grafico .
                        var options = {
                            title: '<%=preguntas[pregunta].getPregunta()%>',
                            is3D: true,
                          };

                        // Instanciar y pintar el grafico
...
%>
    ...
    Información de la Encuesta
            ...
            <div class="card mt-3">
                ...
                <div class="card-body">
                    <h5 class="card-title font-titulo-enc"><%=nombre%></h5>
